...
class JWTService:
    def __init__(self, refresh_token_repository: RefreshTokenService, settings: Settings):
    ...
    def _create_token(self, payload: BaseModel, expire_time) -> str:
        ...
        encoded_jwt = jwt.encode(payload=payload_dict, key=self.config.secret_key, algorithm=self.config.algorithms[0])
        return encoded_jwt
    ...
    async def create_refresh_token(self, payload: RefreshTokenPayloadSchema, session: AsyncSession) -> str:
        new_expiration_time = datetime.now(timezone.utc) + timedelta(days=self.config.refresh_token_expiration_days)
    ...
    def encode(self, payload: dict) -> str:
        return jwt.encode(payload=payload, key=self.config.secret_key, algorithm=self.config.algorithms[0])

    def decode(self, token: str) -> dict:
        return jwt.decode(jwt=token, key=self.config.secret_key, algorithms=self.config.algorithms)
